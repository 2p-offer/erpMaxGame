...
import com.erp.net.utils.ProtoUtil;
...
public class TcpClientHandler extends AbstractNetMsgChannelInboundHandler {
    ...
    public void messageReceived(NettyNetChannel channel, NetMsg msg) {
        ...
        try {
            ...
            System.out.println("收到服务器响应:" + ProtoUtil.toJson(bizResponse));
            ByteString bizData = bizResponse.getData();
            SimpleStringMsg.SimpleStringResponse simpleStringResponse = SimpleStringMsg.SimpleStringResponse.parseFrom(bizData);
            System.out.println("真实业务数据:" + ProtoUtil.toJson(simpleStringResponse));
        } catch (Exception e) {
